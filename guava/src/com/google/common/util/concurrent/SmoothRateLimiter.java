...
abstract class SmoothRateLimiter extends RateLimiter {
  ...
  static final class SmoothBursty extends SmoothRateLimiter {
    ...
    void doSetRate(double permitsPerSecond, double stableIntervalMicros) {
      ...
      System.out.println("SmoothBursty -> doSetRate() -> maxPermits = " + maxPermits);
      System.out.println("SmoothBursty -> doSetRate() -> storedPermits = " + storedPermits);
    }
  ...
  /**
   * 两个单位请求之间的间隔，以我们稳定的速率。 例如，每秒5个许可的稳定速率具有200ms的稳定间隔
   */
  double stableIntervalMicros;

  /**
   * 下一个请求（无论其大小）将被授予的时间。
   * 在批准请求之后，将来请求会进一步推迟。 大量的请求比小量的请求推迟的更远。
   */
  ...
  final void doSetRate(double permitsPerSecond, long nowMicros) {
    ...
    System.out.println("doSetRate() -> stableIntervalMicros = " + stableIntervalMicros);
  ...
  final long reserveEarliestAvailable(int requiredPermits, long nowMicros) {
    ...
    // 还需要的(需要补充的)令牌数（这里的等待其实是再还上一次预支的令牌，本次的预支不需要等待，留给下一次再还）
    double freshPermits = requiredPermits - storedPermitsToSpend;

    // burthy直接返回0,这里主要针对WarmingUp使用
    long waitMicros = storedPermitsToWaitTime(this.storedPermits, storedPermitsToSpend)
        + (long) (freshPermits * stableIntervalMicros);

    // 更新下一次不需要等待时间，本次计算的nextFreeTicketMicros不返回
    // 更新前的(上次请求计算的)nextFreeTicketMicros，而不是本次更新的nextFreeTicketMicros，通俗来讲，本次请求需要为上次请求的预消费行为埋单
    this.nextFreeTicketMicros = nextFreeTicketMicros + waitMicros;
    System.out.println("reserveEarliestAvailable() -> nextFreeTicketMicros = " + this.nextFreeTicketMicros);
    // 减扣消费的令牌数
